...
"""
使用app接口及未登录cookie获取店铺列表

"""


class TbshopspiderSpider(scrapy.Spider):
    name = 'TbShopSpider_logout_app'
    ...
    def __init__(self, **kwargs):
        ...
        self.retry1 = 0  # 抓取店铺详情尝试次数
        self.retry2 = 0  # 搜索店铺名称的尝试次数
    ...
    def get_proxy(self):
        # u = "http://webapi.http.zhimacangku.com/getip?num=1&type=2&pro=&city=0&yys=0&port=11&pack=39529&ts=1&ys=0&cs=0&lb=1&sb=0&pb=4&mr=1&regions="
        # u = "http://webapi.http.zhimacangku.com/getip?num=1&type=2&pro=&city=0&yys=0&port=11&time=1&ts=1&ys=0&cs=0&lb=1&sb=0&pb=4&mr=1&regions="
        u = "http://webapi.http.zhimacangku.com/getip?num=1&type=2&pro=&city=0&yys=0&port=11&time=1&ts=0&ys=0&cs=0&lb=1&sb=0&pb=4&mr=1&regions="
        n = 0
        while n < 5:
            n += 1
            resp = requests.get(u, timeout=5).json()
            data = resp.get('data')[0]
            ip, port = data.get('ip'), data.get('port')
            proxies = {"https": f"https://{ip}:{port}", "http": f"http://{ip}:{port}"}
            r = requests.get('https://www.taobao.com', timeout=5, proxies=proxies)
    ...
    def start_requests(self):
        ...
        # data = {"id": 231, "shop_id": "135410893", "title": "CoCo可可美衣獨家定制", "name": "洋羊韓國小清新", "platform_id": 3, "shop_link": "https://shop135410893.taobao.com/?spm=2013.1.1000126.2.25703ed5oFUTiR", "callback": "http://release.pl.kydev.org/api/screen/shop", "token": "ce934bc118beedabd789ed5cf6a20dc7"}
        ...
        yield scrapy.Request(url=self.baseurl + title, headers=self.taobao_header,
                             ...
                             meta={'sid': sid, 'shopid': shop_id, 'no_crawlera': True})
    ...
    def parse_search(self, response):
        ...
        if not re_dt and self.retry == 0:
            ...
            yield scrapy.Request(url=self.baseurl + self.seller_name, headers=self.taobao_header,
                                 cookies=self.dict_cookies, callback=self.parse_search,
                                 meta={'sid': sid, 'shopid': shopid, 'no_crawlera': True}, dont_filter=True)
            return
        if not re_dt and self.retry == 1:
            self.retry += 1
            seller_name = self.seller_name.replace('店', '')
            yield scrapy.Request(url=self.baseurl + seller_name, headers=self.taobao_header,
                                 cookies=self.dict_cookies, callback=self.parse_search,
                                 meta={'sid': sid, 'shopid': shopid, 'no_crawlera': True}, dont_filter=True)
            return
        if not re_dt and self.retry >= 2:
        ...
        yield scrapy.Request(url=self.store_detail.format(**st),
                             # headers=hd,
    ...
    def parse_store_item(self, response):
        ...
        if not tdt:
            if self.retry1 < 3:
                ...
                cookie = self.get_logout_token()
                hd = self.taobao_header
                hd.update({'cookie': cookie.get('strcookie')})
                data = '{"sellerId": %s}' % re_data.get('sellerid')
                st = self.sign(token=cookie.get('token'), appkey=self.appkeys, t=str(int(time.time() * 1000)),
                               data=data)
                yield scrapy.Request(url=self.store_detail.format(**st),
                                     headers=hd, dont_filter=True,
                                      cookies=cookie.get('dict_cookies'), callback=self.parse_store_item,
                                      meta={'re_data': json.dumps(re_data)})
                return
            else:
                return
        ...
        for _ in range(20):
            ...
            try:
                r = requests.get(url=self.shopping_list.format(**sign), headers=hd, proxies=proxies, timeout=5)
            except:
                continue
            if 'RGV587_ERROR' not in r.text:
                break
        ...
        for pn in range(2, total_page + 1):
            ...
            yield scrapy.Request(url=self.shopping_list.format(**sign), headers=hd, meta={'proxy': proxies['https']},

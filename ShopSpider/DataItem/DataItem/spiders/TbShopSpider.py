...
class TbshopspiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        data = {
            ...
            'shop_id': '142840423',
            'name': '时尚靓丽321',
            'shop_link': 'https://shop142840423.taobao.com/?spm=a1z10.3-c.0.0.23fbef1dXUhkX7'
        }
    ...
    def get_proxy(self):
        ...
        while n < 5:
            ...
            proxies = {"https": f"https://{ip}:{port}", "http": f"https://{ip}:{port}"}
    ...
    def parse_store_item(self, response):
        ...
        pns = pns if pns < 21 else 21

        proxies = self.get_proxy()

        for pn in range(1, pns):
            # cookie_info[list(cookie_info)[int(self.s_id) % len(cookie_info)]]
            cookie = self.cookie_token[pn % len(self.cookie_token)]
            hd = copy.deepcopy(self.taobao_header)
            ...
            # r = requests.get(self.shopping_list.format(**sign), headers=hd, proxies=proxies, timeout=10)
            r = requests.get(self.shopping_list.format(**sign), headers=hd, timeout=10)
